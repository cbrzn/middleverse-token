...
const MiddleverseGold = artifacts.require('MiddleverseGold');
...
contract('MiddleverseGold', function (accounts) {
  const [ holder, recipient, holderDelegatee, recipientDelegatee, other1, other2 ] = accounts;

  const name = 'Middleverse Gold';
  const symbol = 'MVG';
  const version = '1';
  let supply = ethers.utils.parseUnits("500000000", 18);

  beforeEach(async function () {
    this.token = await MiddleverseGold.new();
  ...
  describe('set delegation', function () {
    describe('call', function () {
      it('delegation with balance', async function () {
        await this.token.transfer(holder, supply);
        ...
        expectEvent(receipt, 'DelegateVotesChanged', { 
          delegate: holder,
          previousBalance: '0',
          newBalance: supply.toString(),
        });

        expect(await this.token.delegates(holder)).to.be.equal(holder);

        let votes = await this.token.getVotes(holder);
        votes = votes.toString();

        expect(votes).to.be.equal(supply.toString());
        expect(await this.token.getPastVotes(holder, receipt.blockNumber - 1)).to.be.bignumber.equal('0');
        await time.advanceBlock();

        let pastVotes = await this.token.getPastVotes(holder, receipt.blockNumber);
        pastVotes = pastVotes.toString();

        expect(pastVotes).to.be.equal(supply);
      });
    ...
    describe('with signature', function () {
      ...
      it('accept signed delegation', async function () {
        ...
        let votes = await this.token.getVotes(delegatorAddress);
        votes = votes.toString();
        expect(votes).to.be.equal(supply.toString());
        let pastVotes = await this.token.getPastVotes(delegatorAddress, receipt.blockNumber - 1);
        pastVotes = pastVotes.toString();
        expect(pastVotes).to.be.equal('0');
        await time.advanceBlock();
        pastVotes = await this.token.getPastVotes(delegatorAddress, receipt.blockNumber);
        pastVotes = pastVotes.toString();
        expect(pastVotes).to.be.equal(supply.toString());
      });
  ...
  describe('change delegation', function () {
    ...
    it('call', async function () {
      ...
      expectEvent(receipt, 'DelegateVotesChanged', {
        delegate: holder,
        previousBalance: supply.toString(),
        newBalance: '0',
      });
      expectEvent(receipt, 'DelegateVotesChanged', {
      ...
      let votes = await this.token.getVotes(holder);
      votes = votes.toString();
      let votesHolderDelegatee = await this.token.getVotes(holderDelegatee);
      votesHolderDelegatee = votesHolderDelegatee.toString();

      let pastVotes = await this.token.getPastVotes(holder, receipt.blockNumber - 1);
      pastVotes = pastVotes.toString();
      let pastVotesHolderDelegatee = await this.token.getPastVotes(holderDelegatee, receipt.blockNumber - 1);
      pastVotesHolderDelegatee = pastVotesHolderDelegatee.toString();

      expect(votes).to.be.equal('0');
      expect(votesHolderDelegatee).to.be.equal(supply.toString());
      expect(pastVotes).to.be.bignumber.equal(supply.toString());
      expect(pastVotesHolderDelegatee).to.be.equal('0');
      await time.advanceBlock();

      pastVotes = await this.token.getPastVotes(holder, receipt.blockNumber);
      pastVotes = pastVotes.toString();
      pastVotesHolderDelegatee = await this.token.getPastVotes(holderDelegatee, receipt.blockNumber);
      pastVotesHolderDelegatee = pastVotesHolderDelegatee.toString();

      expect(pastVotes).to.be.bignumber.equal('0');
      expect(pastVotesHolderDelegatee).to.be.equal(supply.toString());
    });
  });
  ...
  describe('transfers', function () {
    ...
    it('sender delegation', async function () {
      ...
      const { receipt } = await this.token.transfer(recipient, 1, { from: holder });
      expectEvent(receipt, 'Transfer', { from: holder, to: recipient, value: '1' });
      expectEvent(receipt, 'DelegateVotesChanged', { delegate: holder, previousBalance: supply.toString(), newBalance: "499999999999999999999999999" });
      ...
      this.holderVotes = "499999999999999999999999999";
      this.recipientVotes = '0';
    });
  ...
  describe('Compound test suite', function () {
    ...
    describe('balanceOf', function () {
      it('grants to initial account', async function () {
        expect(await this.token.balanceOf(holder)).to.be.bignumber.equal('500000000000000000000000000');
      });
    });
    ...
    describe('getPastVotes', function () {
      ...
      it('returns the latest block if >= last checkpoint block', async function () {
        ...
        expect(await this.token.getPastVotes(other1, t1.receipt.blockNumber)).to.be.bignumber.equal('500000000000000000000000000');
        expect(await this.token.getPastVotes(other1, t1.receipt.blockNumber + 1)).to.be.bignumber.equal('500000000000000000000000000');
      });
      ...
      it('generally returns the voting balance at the appropriate checkpoint', async function () {
        ...
        expect(await this.token.getPastVotes(other1, t1.receipt.blockNumber)).to.be.bignumber.equal('500000000000000000000000000');
        expect(await this.token.getPastVotes(other1, t1.receipt.blockNumber + 1)).to.be.bignumber.equal('500000000000000000000000000');
        expect(await this.token.getPastVotes(other1, t2.receipt.blockNumber)).to.be.bignumber.equal('499999999999999999999999990');
        expect(await this.token.getPastVotes(other1, t2.receipt.blockNumber + 1)).to.be.bignumber.equal('499999999999999999999999990');
        expect(await this.token.getPastVotes(other1, t3.receipt.blockNumber)).to.be.bignumber.equal('499999999999999999999999980');
        expect(await this.token.getPastVotes(other1, t3.receipt.blockNumber + 1)).to.be.bignumber.equal('499999999999999999999999980');
        expect(await this.token.getPastVotes(other1, t4.receipt.blockNumber)).to.be.bignumber.equal('500000000000000000000000000');
        expect(await this.token.getPastVotes(other1, t4.receipt.blockNumber + 1)).to.be.bignumber.equal('500000000000000000000000000');
      });
    });
  });
  ...
  describe('getPastTotalSupply', function () {
    ...
    it('returns the latest block if >= last checkpoint block', async function () {
      t1 = await this.token.transfer(holder, supply);


      await time.advanceBlock();
      await time.advanceBlock();

      let pastTokenSupply = await this.token.getPastTotalSupply(t1.receipt.blockNumber);
      pastTokenSupply = pastTokenSupply.toString();

      expect(pastTokenSupply).to.be.equal(supply.toString());

      pastTokenSupply = await this.token.getPastTotalSupply(t1.receipt.blockNumber + 1);
      pastTokenSupply = pastTokenSupply.toString();
    ...
    it('returns zero if < first checkpoint block', async function () {
      ...
      let pastTokenSupply = await this.token.getPastTotalSupply(t1.receipt.blockNumber - 1);
      pastTokenSupply = pastTokenSupply.toString();

      // expect(pastTokenSupply).to.be.equal('0'); will fail cause of premint.

      pastTokenSupply = await this.token.getPastTotalSupply(t1.receipt.blockNumber + 1);
      pastTokenSupply = pastTokenSupply.toString();

      expect(pastTokenSupply).to.be.equal('500000000000000000000000000');
    });
    ...
    it('generally returns the voting balance at the appropriate checkpoint', async function () {
      ...
      expect(await this.token.getPastTotalSupply(t1.receipt.blockNumber - 1)).to.be.bignumber.equal('500000000000000000000000000'); //cause pastTotalSupply is 500M.
      expect(await this.token.getPastTotalSupply(t1.receipt.blockNumber)).to.be.bignumber.equal('500000000000000000000000000');
      expect(await this.token.getPastTotalSupply(t1.receipt.blockNumber + 1)).to.be.bignumber.equal('500000000000000000000000000');
    });
  });
});
